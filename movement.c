...
/*   Updated: 2025/01/30 13:20:01 by zbakour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_player_movement(t_game *game)
{
	...
	if (game->player->items_collected == game->map->coins_count)
	{
		// render_image(game, "textures/open_exit.xpm", game->map->exit_x,
			game->map->exit_y);
			render_image(game, "textures/env/exit_wall.xpm", game->map->exit_x
				- 2, game->map->exit_y - 2);
			render_image(game, "textures/env/exit_door.xpm", game->map->exit_x
				+ 12, game->map->exit_y + 13);
	...
	mlx_put_image_to_window(game->window->mlx, game->window->win,
		game->base_img->img_ptr, 0, 0);
	// for debuging only
	// mlx_string_put(game->window->mlx, game->window->win, new_x, new_y,
		0xFFFFFF, ft_itoa(new_x / 64));
		// mlx_string_put(game->window->mlx, game->window->win, new_x + 40,
			new_y,
		0xFFFFFF, ft_itoa(new_y/ 64));
		return (0);
}

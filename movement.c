...
/*   Updated: 2025/01/25 18:52:58 by zbakour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (new_x < 0 || new_y < 0 || new_x >= (game->map->x * TILE_SIZE) || (new_y >= (game->map->y * TILE_SIZE)))
        return (0);
...
{
    if (game->map->ptr[new_y / 64][new_x / 64] == 'E' && game->player->items_collected == game->map->coins_count)
        return (1);
    return (0);
}
...
{
    int movement_count = game->player->moves_count;
    int digit_x = 10;
    int digit_y = 10;
    int m = 1;
    ...
    while (divisor > 0)
    {
        int digit = movement_count / divisor;
...
{
    int new_x = game->player->x_pos;
    int new_y = game->player->y_pos;
    int old_x = game->player->x_pos;
    int old_y = game->player->y_pos;
    char *direction = NULL;
    ...
    if (game->player->items_collected == game->map->coins_count)
    {
        ...
        // render_image(game, "textures/open_exit.xpm", game->map->exit_x, game->map->exit_y);
        render_image(game, "textures/env/exit_wall.xpm", game->map->exit_x - 2, game->map->exit_y -2);
        render_image(game, "textures/env/exit_door.xpm", game->map->exit_x + 12, game->map->exit_y + 13);
        mlx_put_image_to_window(game->window->mlx, game->window->win, game->base_img->img_ptr, 0, 0);
    }
    ...
    else if (direction && is_move_valid(game, new_x, new_y) && is_the_curse(game, new_x, new_y))
    {
    ...
    else if (direction && is_move_valid(game, new_x, new_y) && is_the_curse_remover(game, new_x, new_y))
    {
    ...
    else if (direction && is_move_valid(game, new_x, new_y) && is_coin(game, new_x, new_y))
    {
    ...
    {
        render_image(game, "textures/bg_64n.xpm", game->player->x_pos, game->player->y_pos);
        render_image(game, "textures/player/p_idle/down/idle_down_00.xpm", game->player->x_pos, game->player->y_pos);
        mlx_put_image_to_window(game->window->mlx, game->window->win, game->base_img->img_ptr, 0, 0);
    }
    mlx_put_image_to_window(game->window->mlx, game->window->win, game->base_img->img_ptr, 0, 0);

    // for debuging only 
    // mlx_string_put(game->window->mlx, game->window->win, new_x, new_y, 0xFFFFFF, ft_itoa(new_x / 64));
    // mlx_string_put(game->window->mlx, game->window->win, new_x + 40, new_y, 0xFFFFFF, ft_itoa(new_y/ 64));
    return (0);
}
